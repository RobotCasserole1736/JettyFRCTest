...
public class signalFileLogger {
    ...
    //THe following 3 methods should only ever be called from one thread
    ...
    //But this method may be calleld from any thread
    public void addSample(Signal sig, double timestamp_s, double val){
        if(log_open){
            Integer idx = logIdxLookup.get(sig);
            if(idx != null){
                synchronized(logLine){
                    ...
                    if(timestamp_s == curTimestamp){
                        ...
                        ///writeData(); TODO Thread safety - this should submit to some queue which gets flushed periodically in the background.
                        Arrays.fill(logLine,"");
                        curTimestamp = timestamp_s;
                        logLine[idx] = Double.toString(val);
                    }
    ...
    private int init(String logPrefix) {
        ...
        //Guard against a development error or some runtime poor sequencing
        if (log_open) {
            System.out.println("Developer Error - log is already open!");
            return -1;
        }
        ...
        try {
            ...
            log_name = OUTPUT_DIR + "log_" + logPrefix + "_" + getDateTimeString() + ".csv";
